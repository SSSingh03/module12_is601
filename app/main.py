...
# FIXED alias (important change here)
@app.post(
    "/users/register",
    response_model=UserResponse,
    status_code=status.HTTP_201_CREATED,
...
def register_user_alias(user_create: UserCreate, db: Session = Depends(get_db)):
    user_data = user_create.dict(exclude={"confirm_password"})
    try:
        user = User.register(db, user_data)
        db.commit()
...
def login_user_alias(user_login: UserLogin, db: Session = Depends(get_db)):
...
# ------------------------------------------------------------------------------
# Calculations Endpoints (unchanged)
# ------------------------------------------------------------------------------
@app.post(
    "/calculations",
